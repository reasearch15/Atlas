...
interface WorkspaceInboxChatPageProps {
  readonly params: Promise<{ readonly chatId: string }>;
}

/**
 * Conversation pane only — workspace shell and chat list remain in parent layouts.
 ...
 */
export default function WorkspaceInboxChatPage({ params }: WorkspaceInboxChatPageProps) {
  ...
  if (!displayConversation) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-[hsl(210_25%_96%)] px-6 text-center">
        ...
        <Button variant="secondary" onClick={() => router.push("/workspace/inbox" as Route)}>
          Back to inbox
        </Button>
      </div>
    );
  ...
  return (
    <ConversationView
      ...
      onBack={() => router.push("/workspace/inbox" as Route)}
    />
  );
}
