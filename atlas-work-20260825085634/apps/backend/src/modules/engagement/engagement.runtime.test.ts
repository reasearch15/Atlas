...
const bank = JSON.parse(
  readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), "data/question-bank.1000.json"), "utf8")
) as EngagementQuestionInput[];
...
describe("engagement cycle", () => {
  ...
  it("can cycle the approved 1000-question bank without repeats", async () => {
    const { runtime } = setup(undefined, bank.slice(0, 12));
...
describe("engagement voting and settlement", () => {
  ...
  it("does not post more than one poll for a slot and persists message ids", async () => {
    ...
    expect(state.chats.get(Number(channelId))?.messages.filter((m) => !m.deleted)).toHaveLength(1);
  });
...
describe("engagement daily declaration", () => {
  ...
  it("sums poll points plus independently aged referrals and declares Top 3 once", async () => {
    ...
    expect(runtime.claims.filter((c) => c.source === "ENGAGEMENT_DAILY")).toHaveLength(2);
